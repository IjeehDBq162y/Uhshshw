...
wait(2)
game.ReplicatedStorage["01_server"]:FireServer("cmd", "-gh 99113422657303 2649326901")
wait(2.5)
...
function SHAKECAM(POSITION,RANGE,INTENSITY,TIME)
	if (Torso.Position - POSITION).Magnitude <= RANGE then
		coroutine.wrap(function()
			local FADER = INTENSITY*5/TIME
			for i = 1, TIME do
				Swait()
				Humanoid.CameraOffset = VT(MRANDOM(-(INTENSITY*5-(FADER*i)),(INTENSITY*5-(FADER*i)))/10,MRANDOM(-(INTENSITY*5-(FADER*i)),(INTENSITY*5-(FADER*i)))/10,MRANDOM(-(INTENSITY*5-(FADER*i)),(INTENSITY*5-(FADER*i)))/10)
			end
...
local ringAnchor = IT("Part")
ringAnchor.Size = VT(0.2,0.2,0.2)
ringAnchor.Transparency = 1
ringAnchor.CanCollide = false
ringAnchor.Anchored = false
ringAnchor.Massless = true
ringAnchor.Parent = Character
local lunw = weld(ringAnchor,ringAnchor,Torso,CF(0,-2,0),d) lunw.Name = "mainweld"
local haloAccessory = Character:FindFirstChild("Accessory (HALOOO)")
if not haloAccessory then
	warn('Lunatic: could not find an accessory named Accessory (HALOOO) on the character - check the exact name in Explorer')
end
local luneye = haloAccessory and haloAccessory:FindFirstChild("Handle")
local lunw2
if luneye then
	for i,v in pairs(Character:GetDescendants()) do
		if (v:IsA("Weld") or v:IsA("Motor6D") or v:IsA("WeldConstraint") or v:IsA("ManualWeld")) and (v.Part0 == luneye or v.Part1 == luneye) then
			v:Destroy()
		end
	end
	luneye.Anchored = false
	luneye.CanCollide = false
	luneye.Massless = true
	lunw2 = weld(ringAnchor,ringAnchor,luneye,CF(0,0,0),d)
end

local OrbAccessory = Character:FindFirstChild("PartAccessory")
local OrbPart = OrbAccessory and OrbAccessory:FindFirstChild("Handle")
local OrbTraveling = false
local OrbTarget
if OrbPart then
	for i,v in pairs(Character:GetDescendants()) do
		if (v:IsA("Weld") or v:IsA("Motor6D") or v:IsA("WeldConstraint") or v:IsA("ManualWeld")) and (v.Part0 == OrbPart or v.Part1 == OrbPart) then
			v:Destroy()
		end
	end
	OrbPart.Anchored = true
	OrbPart.CanCollide = false
	OrbPart.Massless = true
	OrbPart.CFrame = RootPart.CFrame * CF(0,-200,0)
end
...
function lproj()
	...
	if luneye then
	WACKYEFFECT2({Time = 30, EffectType = "Box", Size = VT(1.5,1.5,0), Size2 = VT(0,0,0), Transparency = 0, Transparency2 = 1, CFrame = luneye.CFrame*CF(MRANDOM(-10,10),MRANDOM(-10,10),0), MoveToPos = luneye.Position, RotationX = 0, RotationY = MRANDOM(-5,5), RotationZ = 0, Material = "Neon", Color = scol,SoundID = nil, SoundPitch = nil, SoundVolume = nil, UseBoomerangMath = false, Boomerang = 0, SizeBoomerang = 0})
	WACKYEFFECT2({Time = 60, EffectType = "Sphere", Size = VT(1,1,1), Size2 = VT(8+1*COS(SINE/6),8+1*COS(SINE/6),.1), Transparency = .5, Transparency2 = 1, CFrame = luneye.CFrame, MoveToPos = nil, RotationX = 0, RotationY = 0, RotationZ = 0, Material = "Neon", Color = scol,SoundID = nil, SoundPitch = nil, SoundVolume = nil, UseBoomerangMath = false, Boomerang = 0, SizeBoomerang = 0})
	end
	end
...
function arcs()
	...
	lunc.Anchored = true
	lunc.CanCollide = false
	lunc.Parent = Character
	luneye.Parent = nil
	lunc.CFrame = RightArm.CFrame*CF(0,-1,0)
	coroutine.resume(coroutine.create(function()
		CreateSound(1428541279,RightArm,5,1,false)
	for i = 1,15 do
		Swait()
		lunc.CFrame = lunc.CFrame:lerp(CF(Mouse.Hit.p)*ANGLES(RAD(-90),0,0),.08)
	end
	local hit,pos = Raycast(lunc.Position,lunc.CFrame.lookVector,5,Character)
	...
	if luneye then luneye.Parent = haloAccessory end
	ATTACK = false
end
...
function RingPose()
	if ANIM == "Idle" then
		return CF(-0.4,0,2)*ANGLES(RAD(0),RAD(0),RAD(0))
...
while true do
	Swait()
	if OrbPart then
		if OrbTraveling and OrbTarget then
			OrbPart.CFrame = OrbTarget.CFrame
		else
			OrbPart.CFrame = RootPart.CFrame * CF(0,-200,0)
		end
	end
	...
	if lunw2 then lunw2.C0 = lunw2.C0:lerp(CF(.5*SIN(SINE/70),0-.5*COS(SINE/35),0)*ANGLES(0,0,RAD(SINE*spinspeed)),.1) end
	if luneye then luneye.Color = Color3.fromRGB(33+33*COS(SINE/30),84,185-65*COS(SINE/30)) end
	...
	if ATTACK == false then lunw.C1 = lunw.C1:lerp(RingPose(),.1) end
	unanchor()
